...
matplot(bvec,t(fOpt-fStarOpt),col="black",type="l",lwd=2,xlab="B/BMSY",ylab="Illegal Fishing Effort Response",ylim=range(fOpt))
mtext("c)",line=1)
...
for (i in 1:leverRuns){
  ...
  b = barplot((t(financingCost)/-min(financingCost))[c(2,4,6,8,10)],ylim=range(-1,sum(financingRev[10,])/-min(financingCost)),col=gray.colors(5)[1],xlab="B0/BMSY",ylab="Revenue and Cost NPV")
  par(new=TRUE)
  c = barplot((t(financingRev)/-min(financingCost))[,c(2,4,6,8,10)],ylim=range(-1,sum(financingRev[10,])/-min(financingCost)),col=gray.colors(5)[2:5])
  abline(h=0)
  mtext(paste("Archetype",i),line=1)
  axis(side=1,at=b,labels=seq(0.4,2,.4))
  box()
}
plot.new()
...
mtext("a) Industry NPV versus Landings Tax", line=1,cex=0.75)
...
mtext("b) Break-even point versus Landings Tax", line=1,cex=0.75)
...
mtext("c) Industry NPV versus Enforcement Fine", line=1,cex=0.75)

matplot(vec,breakEvenMat,type="l",lty=c(1,2,3,4,5),lwd=2,col="black",ylab="Break Even Point (Years)",xlab="Enforcement Fine (Scalar of Landings Price)")
mtext("d) Break-even point versus Enforcement Fine", line=1,cex=0.75)
...
mtext("e) Industry NPV versus Tourism Tax", line=1,cex=0.75)
...
mtext("f) Break-even point versus Tourism Tax", line=1,cex=0.75)
...
mtext("g) Industry NPV versus License Fee", line=1,cex=0.75)
...
mtext("h) Break-even point versus License Fee", line=1,cex=0.75)

plot.new()
legend("center",cex=1.5,xpd=NA,title="B0/BMSY",legend=rev(seq(1,bioRuns)/2.5),col="black",lwd=2,lty=rev(c(1,2,3,4,5)))
