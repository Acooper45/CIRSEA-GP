...
mtext("b) Social Planner NPV", line=1)
...
mtext("c) Break-Even Point", line=1)
...
matplot(vec,NPVIndustryMat,type="l",lty=c(1,2,3,4,5),lwd=2,col="black",ylab="Industry NPV (USD)",xlab="Enforcement Fine (Scalar of Fishing Cost)")
mtext("a) Industry NPV", line=1)

matplot(vec,NPVSocialMat,type="l",lty=c(1,2,3,4,5),lwd=2,col="black",ylab="Social Planner NPV (USD)",xlab="Enforcement Fine (Scalar of Fishing Cost)")
mtext("b) Social Planner NPV", line=1)
...
for (i in 1:leverRuns){
  ...
  b = barplot(t(financingCost),ylim=range(NPVIndustryMat,-NPVenfCostMat),col=gray.colors(5)[1],xlab="B0/BMSY",ylab="Revenue and Cost NPV (USD)")
  par(new=TRUE)
  c = barplot(t(financingRev),ylim=range(NPVIndustryMat,-NPVenfCostMat),col=gray.colors(5)[2:5])
  abline(h=0)
