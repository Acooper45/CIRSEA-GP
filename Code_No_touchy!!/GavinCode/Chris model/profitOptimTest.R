...
{  
  ...
  if (bcurrent<=0) QIUU=0 else QIUU = optim(par=0.5,fn=profitPrivateIUUFunc,x=bcurrent,enfEffort=enfEffort,lower=0,upper=bcurrent,method="L-BFGS-B",control=list(fnscale=-1))$par
  
  if (bcurrent-QIUU <= 0) QLegal = 0 else QLegal = f*(bcurrent) - QIUU
