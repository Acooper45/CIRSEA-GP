...
#browser()
        if (pol==1)
        {
          ...
          if (b==0) f1IUU[i]=0 else f1IUU[i] =  max(0,optim(par=0.5,fn=profitPrivateIUUFunc,lower=0,upper=b*K/2,x=b*K/2,enfEffort=e1[i],method="L-BFGS-B",control=list(fnscale=-1))$par/(b*K/2))
        }
