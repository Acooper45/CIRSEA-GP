...
fineExp = function(enfEffort,fine,Q,Qstar,x)
{
  if (Q>Qstar) {
    expect = fineProbFunc(enfEffort) * (Q-Qstar)^2 * fine / x
  }
  else{
    expect = 0
  }
...
{
  if (Q>Qstar & x>0){
    profit = price * Q - cost * Q^2/x - licFee - tax * (Qstar) - fineExp(enfEffort,fine,Q,Qstar,x)
  }
  if(Q<=Qstar & x>0){
...
{
  q = max((price*x/(2*(cost+beta+fineProbFunc(enfE)*fine))),q_star)
  return(q)
}
...
for (i in 1:T)
{
  ...
  enfRev[i] = fineExp(enfEffort[i],fine,Q[i],Qstar[i],biomass[i])
