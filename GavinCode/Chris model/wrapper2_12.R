...
plot.new()
legend("left",cex=2,xpd=NA,title="Archetype",legend=seq(1,3,1),col="black",lwd=2,lty=c(1,2,3))

matplot(bvec,t(fOpt),col="black",type="l",lwd=2,xlab="B/BMSY",ylab="Total Fishing Effort Response",ylim=range(fOpt))
mtext("c)",line=1)
...
for (i in 1:leverRuns){
  ...
  b = barplot(t(financingCost)/-min(financingCost),ylim=range(-1,sum(financingRev[5,])/-min(financingCost)),col=gray.colors(5)[1],xlab="B0/BMSY",ylab="Revenue and Cost NPV")
  par(new=TRUE)
  c = barplot(t(financingRev)/-min(financingCost),ylim=range(-1,sum(financingRev[5,])/-min(financingCost)),col=gray.colors(5)[2:5])
  abline(h=0)
  mtext(paste("Archetype",i),line=1)
  axis(side=1,at=b,labels=seq(1,bioRuns)/2.5)
  box()
}
plot.new()
