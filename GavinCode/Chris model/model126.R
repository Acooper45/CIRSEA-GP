...
{
  if (x==0) qIUU = 0 else qIUU = (price - fineProbFunc(enfEffort)*fine)*x/(2*(cost+beta))
  if (qIUU<0) qIUU=0
  if(qIUU>x) qIUU=x
  return(qIUU)
#  return(0)
...
#   if ((price * qIUU - cost * (qIUU/x)^2 - fineProbFunc(enfEffort) * fine * (qIUU/x)^2)<0) qIUU=0
#   if(qIUU>x) qIUU=x
#   return(qIUU)
# }
...
for (i in 1:T)
{
  ...
  if (archetype == 1) profitTourism[i] =0  else profitTourism[i]=(1-w[i])*tourismRevenueOptimal(biomass[i] - QLegal[i] - QIUU[i])
