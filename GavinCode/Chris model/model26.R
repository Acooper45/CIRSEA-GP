...
{
  q = max((price - tax*price - fineProbFunc(enfE)*fine)*x/(2*(cost+beta)),q_star)
  return(q)
}
...
for (i in 1:T)
{
  ...
  profitFishing[i]=profitPrivateFunc(Qstar[i],Q[i],biomass[i],LF[i],v[i]/price,enfEffort[i],fine)
