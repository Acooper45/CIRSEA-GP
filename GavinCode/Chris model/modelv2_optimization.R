...
## Choose Policy
policy = 1 # 1: FMSY at all time steps; 2: 0 until biomass reaches BMSY, FMSY afterwards; 3: Optimal policy
...
for (i in 1:T)
{
  ...
  QIUU[i] = approx(bvec,optimal$fIUU,(biomass[i] - QLegal[i])/(K/2))$y * (biomass[i] - QLegal[i])
