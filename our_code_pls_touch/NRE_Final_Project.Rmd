...
objective <- function(I, int_stock, discount, p, L, c, ce, r, K, period, effort, alpha) {
  ...
  #initial values
  stock[1] = int_stock
  benefits[1] = p*(L+I[1]) - c*((L+I[1])^2/stock[1]) - ce*(effort) - alpha*effort*I[1]
  ...
  for (i in 2:(period+1))  
    ...
    benefits[i] = p*(L + I[i]) - c*((L+I[i])^2/stock[i]) 
...
output_new = nloptr(x0=rep(1, times=76),
                 ...
                 lb=rep(0, times=76),
                 opts = options,
                 int_stock=8000, discount=.05, p=10, c=7, r=.2, K=2265128, 
                 period =75)
